...
namespace SpatialTools
{
	...
	UEBRIDGEMCPLEGACYTOOLGROUPS_API FBox GetActorWorldBounds(const AActor* Actor);
	...
	UEBRIDGEMCPLEGACYTOOLGROUPS_API bool IsSpatiallyMeaningfulActor(const AActor* Actor, const FBox& ActorBounds);

	// Union of every spatially-meaningful actor's bounds. OutConsidered counts contributors.
	UEBRIDGEMCPLEGACYTOOLGROUPS_API FBox ComputeLevelBounds(UWorld* World, int32& OutConsidered);

	// Compact {min,max,center,size} integer arrays for a box; empty object if the box is invalid.
	UEBRIDGEMCPLEGACYTOOLGROUPS_API TSharedPtr<FJsonObject> BoxToJson(const FBox& Box);
	...
	UEBRIDGEMCPLEGACYTOOLGROUPS_API FString ClassifyActor(const AActor* Actor);
	...
	UEBRIDGEMCPLEGACYTOOLGROUPS_API TArray<TSharedPtr<FJsonValue>> BuildRegionSummaries(UWorld* World, const FBox& LevelBounds, int32 GridDivisions, int32 MaxRegions);
	...
	UEBRIDGEMCPLEGACYTOOLGROUPS_API void ForEachWorldInstance(UWorld* World, TFunctionRef<void(const AActor* Owner, int32 InstanceIndex, const FBox& WorldBounds, const FString& Category)> Visit);
	...
	UEBRIDGEMCPLEGACYTOOLGROUPS_API void GatherPlacementBounds(UWorld* World, TArray<FBox>& OutBounds, const FBox& RegionFilter = FBox(ForceInit));
