	...
	const TSharedPtr<FJsonObject>& MetadataObject)
{
	...
	for (const auto& Pair : MetadataObject->Values)
	{
		...
		{
			const FString MetadataKey = UEBridgeMCPJson::KeyToString(Pair.Key);
			SetOrClearBlueprintVariableMetaData(Blueprint, VarName, FName(*MetadataKey), MetadataValue);
		}
	}
}
	...
	const TSharedPtr<FJsonObject>& MetadataObject)
{
	...
	for (const auto& Pair : MetadataObject->Values)
	{
		...
		{
			const FString MetadataKey = UEBridgeMCPJson::KeyToString(Pair.Key);
			SetOrClearVariableDescriptionMetaData(VariableDescription, FName(*MetadataKey), MetadataValue);
		}
	}
}
	...
	if (Category == UEdGraphSchema_K2::PC_Struct)
	{
		...
		if (JsonValue->TryGetObject(ObjVal) && ObjVal && (*ObjVal).IsValid())
		{
			...
			for (const auto& Pair : (*ObjVal)->Values)
			{
				...
				const FString Key = UEBridgeMCPJson::KeyToString(Pair.Key);
				Result += FString::Printf(TEXT("%s=%s"), *Key, *Pair.Value->AsString());
				bFirst = false;
			}
			Result += TEXT(")");
			return Result;
