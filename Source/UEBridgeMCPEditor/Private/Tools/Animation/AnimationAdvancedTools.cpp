...
namespace
{
	...
	bool ApplyPropertyMap(UObject* Target, const TSharedPtr<FJsonObject>& PropertiesObject, bool bApply, FString& OutError)
	{
		...
		{
			const FString PropertyName = UEBridgeMCPJson::KeyToString(Pair.Key);
			FProperty* Property = nullptr;
			void* Container = nullptr;
			FString PropertyError;
			if (!FMcpAssetModifier::FindPropertyByPath(Target, PropertyName, Property, Container, PropertyError))
			{
				OutError = FString::Printf(TEXT("Property '%s' not found: %s"), *PropertyName, *PropertyError);
				return false;
			}
			if (bApply && !FMcpAssetModifier::SetPropertyFromJson(Property, Container, Pair.Value, PropertyError))
			{
				OutError = FString::Printf(TEXT("Failed to set property '%s': %s"), *PropertyName, *PropertyError);
				return false;
			}
		}
		return true;
