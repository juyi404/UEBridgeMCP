	...
	FString& OutError)
{
	...
	{
		const FString PropertyPath = UEBridgeMCPJson::KeyToString(Pair.Key);
		FProperty* Property = nullptr;
		void* Container = nullptr;
		FString PropertyError;
		if (!FMcpAssetModifier::FindPropertyByPath(TargetObject, PropertyPath, Property, Container, PropertyError))
		{
	...
	FString& OutError)
{
	...
	{
		const FString PropertyPath = UEBridgeMCPJson::KeyToString(Pair.Key);
		if (CanvasKeys.Contains(PropertyPath))
		{
			continue;
		}
		...
		void* Container = nullptr;
		FString PropertyError;
		if (!FMcpAssetModifier::FindPropertyByPath(Slot, PropertyPath, Property, Container, PropertyError))
		{
			OutError = FString::Printf(TEXT("Slot property '%s' not found: %s"), *PropertyPath, *PropertyError);
			return false;
		}

		if (bApply && !FMcpAssetModifier::SetPropertyFromJson(Property, Container, Pair.Value, PropertyError))
		{
			OutError = FString::Printf(TEXT("Failed to set slot property '%s': %s"), *PropertyPath, *PropertyError);
			return false;
		}
	}
