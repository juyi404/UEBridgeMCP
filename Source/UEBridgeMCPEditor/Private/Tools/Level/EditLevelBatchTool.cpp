...
namespace EditLevelBatchToolPrivate
{
	...
	bool ApplyPropertyOverrides(UObject* Object, const TSharedPtr<FJsonObject>& PropertiesObject, FString& OutError)
	{
		...
		for (const auto& Pair : PropertiesObject->Values)
		{
			...
			if (!FMcpAssetModifier::FindPropertyByPath(Object, PropertyPath, Property, Container, PropertyError))
			{
				OutError = FString::Printf(TEXT("Failed to find property '%s': %s"), *PropertyPath, *PropertyError);
				return false;
			}
