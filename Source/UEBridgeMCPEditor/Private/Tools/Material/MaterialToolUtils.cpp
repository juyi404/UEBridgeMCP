...
namespace MaterialToolUtils
{
	...
	bool ApplyObjectProperties(UObject* Object, const TSharedPtr<FJsonObject>& Properties, FString& OutError)
	{
		...
		for (const auto& Pair : Properties->Values)
		{
			...
			{
				FString FindError;
				if (!FMcpAssetModifier::FindPropertyByPath(Object, PropertyName, Property, Container, FindError))
				{
					OutError = FString::Printf(TEXT("Failed to find property '%s': %s"), *PropertyName, *FindError);
					return false;
				}
			}
			...
			{
				OutError = FString::Printf(TEXT("Failed to set property '%s': %s"), *PropertyName, *SetError);
				return false;
			}
		}
