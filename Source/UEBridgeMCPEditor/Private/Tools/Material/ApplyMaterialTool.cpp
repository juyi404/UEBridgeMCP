...
namespace ApplyMaterialToolPrivate
{
		...
		FString& OutError)
	{
		...
		for (const auto& Pair : ParameterOverrides->Values)
		{
			const FString ParameterKey = UEBridgeMCPJson::KeyToString(Pair.Key);
			const FName ParameterName(*ParameterKey);
			...
			else if (Value->Type == EJson::Array)
			{
				...
				if (ArrayValue.Num() < 3)
				{
					OutError = FString::Printf(TEXT("Parameter '%s' requires at least 3 numbers for vector values"), *ParameterKey);
					return false;
				}
			...
			{
				OutError = FString::Printf(TEXT("Unsupported parameter override value type for '%s'"), *ParameterKey);
				return false;
			}
		}
