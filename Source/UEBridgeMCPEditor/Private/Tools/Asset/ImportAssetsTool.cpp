...
#include "Compat/UEBridgeMCPVersionCompat.h"
...
namespace ImportAssetsToolPrivate
{
	...
	bool ApplyImportOptions(const TArray<UObject*>& TargetObjects, const TSharedPtr<FJsonObject>& ImportOptionsObject, FString& OutError)
	{
		...
		for (const auto& Pair : ImportOptionsObject->Values)
		{
			const FString OptionName = UEBridgeMCPJson::KeyToString(Pair.Key);
			bool bApplied = false;
			...
			for (UObject* TargetObject : TargetObjects)
			{
				...
				FString EnsureError;
				if (!EnsureObjectPropertyChain(TargetObject, OptionName, EnsureError))
				{
				...
				if (!FMcpAssetModifier::FindPropertyByPath(TargetObject, OptionName, Property, Container, PropertyError))
				{
				...
				{
					OutError = FString::Printf(TEXT("Failed to set import option '%s': %s"), *OptionName, *PropertyError);
					return false;
				}
			...
			if (!bApplied)
			{
				OutError = FString::Printf(TEXT("Failed to resolve import option '%s': %s"), *OptionName, *LastResolveError);
				return false;
			}
		}
	...
	for (int32 ImportIndex = 0; ImportIndex < ImportsArray->Num(); ++ImportIndex)
	{
		...
		if (ImportOptionsObject && (*ImportOptionsObject).IsValid())
		{
			...
			if (ImportTask->Factory)
			{
				...
				{
					const FString OptionName = UEBridgeMCPJson::KeyToString(Pair.Key);
					if (!ImportAssetsToolPrivate::EnsureObjectPropertyChain(ImportTask->Factory, OptionName, OptionsError))
					{
						break;
					}
				}
