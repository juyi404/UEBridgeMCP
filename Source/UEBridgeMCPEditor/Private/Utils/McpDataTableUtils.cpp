	...
	FString& OutError)
{
	...
	{
		const FString FieldName = UEBridgeMCPJson::KeyToString(Pair.Key);
		FProperty* Property = FindPropertyByName(RowStruct, FieldName);
		if (!Property)
		{
			OutError = FString::Printf(TEXT("Unknown row field '%s' for struct '%s'"), *FieldName, *RowStruct->GetPathName());
			return false;
		}
		...
		OutError = FString::Printf(TEXT("Failed to deserialize row field '%s': %s"), *FieldName, *DeserializeError);
		return false;
	}
